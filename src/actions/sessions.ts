...
export async function getSessionById(sessionId: string) {
  try {
    ...
    return await db.query.sessions.findFirst({
      where: (sessions, { eq, and }) => and(eq(sessions.id, sessionId)),
      with: {
        user: true,
        likes: {
          with: { user: true },
...
export async function deleteSession(sessionId: string) {
  ...
  await db.delete(sessions).where(and(eq(sessions.id, sessionId), eq(sessions.userId, user[0].id)));
...
export async function getFeed() {
  ...
  const following = await db.select().from(follows).where(eq(follows.followerId, user[0].id));
