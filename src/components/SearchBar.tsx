...
export default function SearchBar({ currentUserId, followingIds }: SearchBarProps) {
    ...
    return(
        <div className="relative mb-8">
            ...
            />
            {loading && (
                <p className="text-text-secondary text-xs mt-2">Searching...</p>
            )}
            {results.length > 0 && (
                <div className="absolute top-full left-0 right-0 mt-1 bg-background border border-border rounded-lg overflow-hidden z-10">
                    {results.map((user) => (
                        <div key={user.id} className="flex items-center justify-between p-3 hover:bg-surface transition-colors border-b border-border last:border-0">
